...
- \`lib/auth.ts\`: \`import { betterAuth } from "better-auth"; import { Pool } from "pg"; export const auth = betterAuth({ database: new Pool({ connectionString: process.env.DATABASE_URL }), emailAndPassword: { enabled: true }, secret: process.env.BETTER_AUTH_SECRET, baseURL: process.env.BETTER_AUTH_URL, trustedOrigins: process.env.BETTER_AUTH_URL ? [process.env.BETTER_AUTH_URL] : [], advanced: { defaultCookieAttributes: { sameSite: process.env.EMERGENT_PREVIEW_IFRAME === "1" ? "none" : "lax", secure: true } } });\` — read the secret from process.env.BETTER_AUTH_SECRET and the app's public origin from process.env.BETTER_AUTH_URL; BOTH are already in \`.env.local\`. Never hardcode or self-generate either. Setting baseURL + trustedOrigins from BETTER_AUTH_URL is REQUIRED: the live preview is served through a proxy, so better-auth's header-inferred origin does not match the external preview URL and every sign-up/sign-in POST fails with "Invalid origin" unless you pin them from this env var. The \`advanced.defaultCookieAttributes\` block is REQUIRED too, and its \`sameSite\` MUST stay conditional exactly as written. better-auth defaults to \`sameSite: "lax"\`, and the live preview is displayed inside an iframe on a different origin, so a Lax cookie is never sent back there: login appears to succeed and every following request is anonymous. But SameSite=Lax is also a CSRF protection, and this same code gets deployed to a normal top-level domain later, where the iframe no longer exists and the relaxation would be pure risk. EMERGENT_PREVIEW_IFRAME is set only inside the sandbox's \`.env.local\`, so the app relaxes the cookie in the preview and keeps Lax in production. Do NOT hardcode \`sameSite: "none"\`. Both environments are served over HTTPS, so \`secure: true\` is correct in each.
...
- \`lib/auth.ts\` missing \`advanced.defaultCookieAttributes\` entirely. better-auth then defaults to Lax, which a browser will not send back inside the preview iframe, so every request after login is anonymous.
- \`lib/auth.ts\` hardcoding \`sameSite: "none"\` instead of gating it on \`process.env.EMERGENT_PREVIEW_IFRAME === "1"\`. That flag exists only in the sandbox, so a hardcoded "none" ships to production and drops SameSite's CSRF protection on a site that never needed the relaxation. Report it.
