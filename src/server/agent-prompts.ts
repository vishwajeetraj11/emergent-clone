...
- Route protection goes in \`proxy.ts\`, NOT \`middleware.ts\`. In this version of Next.js the \`middleware\` file convention is DEPRECATED and renamed to \`proxy\` — a file named \`middleware.ts\` is not a convention here, is never executed, and silently leaves every "protected" route unguarded. Put \`proxy.ts\` beside \`app/\` (project root, or inside \`src/\` if the app lives there), export a function named \`proxy\` (or a default export), and export \`const config = { matcher: [...] }\`. Read the session inside it with a plain cookie check or a fetch to your own auth route; do NOT import \`lib/auth.ts\` there, since proxy runs in a separate, restricted context.
...
- A \`middleware.ts\` file. In this version of Next.js that convention is deprecated and renamed to \`proxy\`, so the file never executes and whatever it was protecting is unguarded. Report it: it must be \`proxy.ts\` exporting \`proxy\` plus \`config.matcher\`.
